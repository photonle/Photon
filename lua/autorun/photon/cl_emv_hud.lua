...
local function EMV_Hud()
	...
	local srSET = EMV:Photon_SirenSet()
	local elOPT = EMV:Photon_LightOption()
	if not elOPT then elOPT = 1 end
	if not EMV:Photon_ELS_GetSequenceName() then error("Could not get sequence name: " .. tostring( EMV:Photon_ELS_GetSequenceName() )) return end
	local lightName = EMV:Photon_ELS_GetSequenceName()
	if not lightName then lightName = "UNKNOWN" end
	local srOPT = EMV:Photon_SirenOption()
	if not srOPT then srOPT = 1 end
	local sirenName = ""
	if not EMV:Photon_NoSiren() then sirenName = EMVU.Sirens[srSET]["Set"][srOPT]["Name"] end
	if not sirenName then sirenName = "UNKNOWN" end

	local sirenSet = ""
	if not EMV:Photon_NoSiren() then sirenSet = EMVU.Sirens[srSET]["Name"] end
	if not sirenSet then sirenSet = "" end

	local illumOpt = ""
	if EMV:Photon_HasIllum() then illumOpt = tostring( EMVU.Helper:GetIlluminationName( EMV.VehicleName, EMV:Photon_IllumOption() ) ) end
	
	local trafficOpt = ""
	if EMV:Photon_HasTrafficAdvisor() then trafficOpt = tostring( EMVU.Helper:GetTrafficAdvisorName( EMV.VehicleName, EMV:Photon_TrafficAdvisorOption() )) end

	local heightMin = 160
	if EMV:Photon_HasIllum() then heightMin = heightMin + 40 end
	if EMV:Photon_HasTrafficAdvisor() then heightMin = heightMin + 40 end
	bY = ScrH() - heightMin
	...
	if not EMV:Photon_NoSiren() then
		local height = 124
		if EMV:Photon_HasIllum() then height = height + 40 end
		if EMV:Photon_HasTrafficAdvisor() then height = height + 40 end
	...
	if not EMV:Photon_NoSiren() then draw.DrawText( "SIREN", "EMV_Title", rX + 10, rY + 8, Color(255,255,255,255), TEXT_ALIGN_LEFT) end -- SIREN title
	...
	if EMV:Photon_HasTrafficAdvisor() then
		...
		rY = rY + 40
		draw.RoundedBox( 4, rX, rY, 52, 24, Color(24,24,24,255))
		if EMV:Photon_TrafficAdvisor() then
			draw.RoundedBox(2, rX + 4, rY + 4, 16, 16, Color(136,210,104,255))
			draw.DrawText("ON", "EMV_Switch", rX + 27, rY + 4, Color(255,255,255,255), TEXT_ALIGN_LEFT)
		else
			draw.RoundedBox(2, rX + 32, rY + 4, 16, 16, Color(255,255,255,50))
	...
	draw.RoundedBox( 0, bX, rY + 35, 160, 21, Color(24,24,24,255))
		...
		if EMV.IsRunning and EMV:Photon_IsRunning() then
		...
		if EMV.ManualHorn and EMV:Photon_ManualHorn() then
		...
		if EMV.ManualSiren and EMV:Photon_ManualSiren() then
...
function PhotonHUD()
	...
	if (car:Photon_TurningLeft() or car:Photon_Hazards()) and car:Photon_BlinkOn() then
	...
	if (car:Photon_TurningRight() or car:Photon_Hazards()) and car:Photon_BlinkOn()  then
		surface.SetDrawColor( 136, 210, 104 )
	else
		surface.SetDrawColor( 255, 255, 255, 40 )
	end
	...
	if car:Photon_HeadlightsOn() then
	...
	if car:Photon_Hazards() and not car:Photon_BlinkOn() then
